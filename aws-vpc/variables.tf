...
variable "container_subnets" {
  description = "A list of container subnets inside the VPC"
  type        = list(string)
  default     = []
}

variable "container_subnet_assign_ipv6_address_on_creation" {
  description = "Specify true to indicate that network interfaces created in the specified subnet should be assigned an IPv6 address. Default is `false`"
  type        = bool
  default     = false
}

variable "container_subnet_enable_dns64" {
  description = "Indicates whether DNS queries made to the Amazon-provided DNS Resolver in this subnet should return synthetic IPv6 addresses for IPv4-only destinations. Default: `true`"
  type        = bool
  default     = true
}

variable "container_subnet_enable_resource_name_dns_aaaa_record_on_launch" {
  description = "Indicates whether to respond to DNS queries for instance hostnames with DNS AAAA records. Default: `true`"
  type        = bool
  default     = true
}

variable "container_subnet_enable_resource_name_dns_a_record_on_launch" {
  description = "Indicates whether to respond to DNS queries for instance hostnames with DNS A records. Default: `false`"
  type        = bool
  default     = false
}

variable "container_subnet_ipv6_prefixes" {
  description = "Assigns IPv6 container subnet id based on the Amazon provided /56 prefix base 10 integer (0-256). Must be of equal length to the corresponding IPv4 subnet list"
  type        = list(string)
  default     = []
}

variable "container_subnet_ipv6_native" {
  description = "Indicates whether to create an IPv6-only subnet. Default: `false`"
  type        = bool
  default     = false
}

variable "container_subnet_private_dns_hostname_type_on_launch" {
  description = "The type of hostnames to assign to instances in the subnet at launch. For IPv6-only subnets, an instance DNS name must be based on the instance ID. For dual-stack and IPv4-only subnets, you can specify whether DNS names use the instance IPv4 address or the instance ID. Valid values: `ip-name`, `resource-name`"
  type        = string
  default     = null
}

variable "container_subnet_names" {
  description = "Explicit values to use in the Name tag on container subnets. If empty, Name tags are generated"
  type        = list(string)
  default     = []
}

variable "container_subnet_suffix" {
  description = "Suffix to append to container subnets name"
  type        = string
  default     = "db"
}

variable "create_container_subnet_route_table" {
  description = "Controls if separate route table for container should be created"
  type        = bool
  default     = false
}

variable "create_container_internet_gateway_route" {
  description = "Controls if an internet gateway route for public container access should be created"
  type        = bool
  default     = false
}

variable "create_container_nat_gateway_route" {
  description = "Controls if a nat gateway route should be created to give internet access to the container subnets"
  type        = bool
  default     = false
}

variable "container_route_table_tags" {
  description = "Additional tags for the container route tables"
  type        = map(string)
  default     = {}
}

variable "container_subnet_tags" {
  description = "Additional tags for the container subnets"
  type        = map(string)
  default     = {}
}

variable "create_container_subnet_group" {
  description = "Controls if container subnet group should be created (n.b. container_subnets must also be set)"
  type        = bool
  default     = true
}

variable "container_subnet_group_name" {
  description = "Name of container subnet group"
  type        = string
  default     = null
}

variable "container_subnet_group_tags" {
  description = "Additional tags for the container subnet group"
  type        = map(string)
  default     = {}
}
...
variable "container_dedicated_network_acl" {
  description = "Whether to use dedicated network ACL (not default) and custom rules for container subnets"
  type        = bool
  default     = false
}

variable "container_inbound_acl_rules" {
  description = "container subnets inbound network ACL rules"
  type        = list(map(string))
  default = [
    {
      rule_number = 100
...
variable "container_outbound_acl_rules" {
  description = "container subnets outbound network ACL rules"
  type        = list(map(string))
  default = [
    {
      rule_number = 100
...
variable "container_acl_tags" {
  description = "Additional tags for the container subnets network ACL"
  type        = map(string)
  default     = {}
}
